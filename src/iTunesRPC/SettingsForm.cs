...
public sealed class SettingsForm : Form
{
    ...
    public SettingsForm(AppConfig config, string configPath)
    {
        ...
        ClientSize = new Size(S(400), S(500));
        Font = new Font("Segoe UI", 9.5f);
        Padding = new Padding(S(12));

        var clientIdLabel = new Label { Text = "Discord Application ID", Left = S(16), Top = S(14), Width = S(360), AutoSize = true };
        _clientIdBox.Left = S(16); _clientIdBox.Top = S(36); _clientIdBox.Width = S(368); _clientIdBox.Height = S(24);
        ...
        _broadcastBox.Left = S(16); _broadcastBox.Top = S(72); _broadcastBox.Width = S(368); _broadcastBox.AutoSize = true;
        _broadcastBox.Checked = config.BroadcastEnabled;

        _trackNumberBox.Text = "Show track number (e.g. Track 2 / 11)";
        _trackNumberBox.Left = S(16); _trackNumberBox.Top = S(98); _trackNumberBox.Width = S(368); _trackNumberBox.AutoSize = true;
        _trackNumberBox.Checked = config.ShowTrackNumber;

        var artSectionLabel = new Label
        {
            Text = "ALBUM ART",
            Left = S(16), Top = S(134), AutoSize = true,
            Font = new Font("Segoe UI", 8.5f, FontStyle.Bold),
            ForeColor = Theme.MutedText
        };

        var artPanel = new Panel { Left = S(16), Top = S(156), Width = S(368), Height = S(190), BorderStyle = BorderStyle.FixedSingle };

        _artAutoRadio.Text = "Automatic (look up cover art)";
        _artAutoRadio.Left = S(12); _artAutoRadio.Top = S(12); _artAutoRadio.Width = S(340); _artAutoRadio.AutoSize = true;

        _artCustomRadio.Text = "Custom image URL (512x512 recommended)";
        _artCustomRadio.Left = S(12); _artCustomRadio.Top = S(40); _artCustomRadio.Width = S(340); _artCustomRadio.AutoSize = true;
        ...
        _artOffRadio.Left = S(12); _artOffRadio.Top = S(98); _artOffRadio.Width = S(340); _artOffRadio.AutoSize = true;
        ...
        var pollLabel = new Label { Text = "Poll interval (ms)", Left = S(16), Top = S(364), AutoSize = true };
        _pollIntervalBox.Left = S(16); _pollIntervalBox.Top = S(386); _pollIntervalBox.Width = S(130); _pollIntervalBox.Height = S(24);
        ...
        _autoLaunchBox.Left = S(16); _autoLaunchBox.Top = S(426); _autoLaunchBox.Width = S(368); _autoLaunchBox.AutoSize = true;
        _autoLaunchBox.Checked = AutoLaunch.IsEnabled;

        _saveButton.Text = "Save";
        _saveButton.Left = S(212); _saveButton.Top = S(460); _saveButton.Width = S(84); _saveButton.Height = S(30);
        _saveButton.Click += (_, _) => Save();

        var closeButton = new Button { Text = "Close", Left = S(304), Top = S(460), Width = S(84), Height = S(30) };
        ...
        {
            clientIdLabel, _clientIdBox, _broadcastBox, _trackNumberBox, artSectionLabel, artPanel,
